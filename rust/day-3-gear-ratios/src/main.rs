    ...
    for (line_idx, line) in vectorized_lines.iter().enumerate() {
        for (char_idx, ch) in line.iter().enumerate() {
            if ch.is_digit(10) == false && ch != &'.' {
                if let Some(num_at_top) = check_top(&vectorized_lines, &line_idx, &char_idx) {
                    accum = accum + num_at_top as usize;
                }
                if let Some(num_at_top) = check_bottom(&vectorized_lines, &line_idx, &char_idx) {
                    accum = accum + num_at_top as usize;
                }
                if let Some(num_at_right) = check_right(&line, &char_idx) {
                    accum = accum + num_at_right as usize;
                }
                if let Some(num_at_left) = check_left(&line, &char_idx) {
...
fn check_top(vectorized_lines: &Vec<Vec<char>>, line_idx: &usize, symbol_idx: &usize) -> Option<u16> {
    if line_idx == &0 {
        return None;
    }
...
fn check_bottom(vectorized_lines: &Vec<Vec<char>>, line_idx: &usize, symbol_idx: &usize) -> Option<u16> {
...
fn check_right(current_line: &Vec<char>, symbol_idx: &usize) -> Option<u16> {
...
fn check_left(current_line: &Vec<char>, symbol_idx: &usize) -> Option<u16> {
    let char_at_right = current_line.get(*symbol_idx - 1);
    if char_at_right.is_none() || char_at_right.unwrap().is_digit(10) == false {
        return None;
    }
...
fn extract_number_based_on_index(vectorized_line: &Vec<char>, char_num_idx: usize) -> u16 {
